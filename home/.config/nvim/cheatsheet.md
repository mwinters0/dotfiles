...
## LSP
- `:LspInfo`
- `:LspLog` (should be `~/.local/state/nvim/lsp.log`)
- `:help lspconfig-all`
- `:lsp restart` / `:lsp stop` + `:edit`
...
- LSP
  - project-wide code actions, e.g. rename / refactor
    ...
    - ? https://github.com/python-rope/rope (included in python-lsp-server)
  - k8s LSP
  - JSONSchema (`jsonls`, `yamlls`)
...
- Pairs
  - https://github.com/nvim-mini/mini.pairs/tree/main
    - TODO: investigate config options.  Too aggressive.
...
- Terminal
  - https://github.com/nvzone/floaterm (nvchad)
...
- ? Vertically stacked (:split) explorers?  Useful to e.g. browse abstract classes vs implementations
  - Or explorer tabs ...
